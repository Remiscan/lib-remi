...
const resizeObserver = new ResizeObserver((entries) => {
    for (const entry of entries) {
        if (entry.borderBoxSize?.length > 0) {
            ...
            if (!(section instanceof ScrollZoomBlock))
                continue;
            ...
            section.initializeScrollPosition({ inline: sectionInlineSize, block: sectionBlockSize }, { inline: contentInlineSize, block: contentBlockSize });
        }
    }
});
/**
...
export class ScrollZoomBlock extends HTMLElement {
    ...
    get zoomInButtonSlot() {
        ...
        if (!(slot instanceof HTMLSlotElement))
            throw new TypeError('Expecting HTMLSlotElement');
        return slot;
    }
    /** Zoom-in button. */
    get zoomInButton() {
     ...
     */
    #computeScrollMargins(zoomLevel, sectionSize = this.#size, contentSize = this.#contentSize) {
     ...
     */
    initializeScrollPosition(sectionSize, contentSize) {
    ...
    updateZoomRangeSlider() {
        this.isInitialized.then(() => {
            ...
            if ('value' in slider && slider.value != newSliderValue)
    ...
    anyPointerWasDoubleTapDrag(pointers) {
        for (const pointer of pointers) {
            if (pointer.becameDoubleTapDrag)
                return true;
        }
        return false;
    }
    ...
    onPointerDown(downEvent) {
        ...
        if (!doubleTapPanHappening) {
            ...
            for (const [pointerId, evt] of this.currentPointerDownEvents.entries()) {
                ...
                if (mvEvt)
                    pinchPointerEvents.add(mvEvt);
                // If not, use its `pointerdown` event
                else
                    pinchPointerEvents.add(evt);
            }
            this.lastPinchData = {
            ...
            case 'pinch':
                {
                    ...
                    if (!lastPinchData)
                        throw new Error(`Cannot perform ${interaction} interaction because of missing pinch data`);
                    ...
                    const scrollPosition = this.scrollPosition
                        .translate(-(centerPoint.x - lastPinchData.centerPoint.x), -(centerPoint.y - lastPinchData.centerPoint.y))
                        .round();
                    this.scrollableContainer.scrollTo({
                        left: scrollPosition.x,
                        top: scrollPosition.y,
            ...
            case 'double-tap-pan':
                {
                    ...
                    this.zoom(this.lastPinchData.startZoomLevel * zoomScale, undefined, undefined, this.lastPinchData.startZoomLevel, downEvent.startScrollPosition);
                    this.dispatchInteractionEvent('after', interaction, interactionDetail);
                }
                break;
            default:
        ...
        switch (interaction) {
            ...
            case 'double-tap':
                {
                    ...
                    const { promise, abortController } = this.smoothZoom(this.getNextZoomLevel(zoomDirection), zoomPoint);
            ...
            default: {
                ...
                if (downEvent.lastInteraction === 'pan' || downEvent.lastInteraction === 'pinch') {
                    ...
                    this.inertiaTracker.startInertia(this.velocityTracker.velocity, (displacement) => {
    ...
    onWheel(event) {
        ...
        this.dispatchInteractionEvent('before', interaction, interactionDetail);
        this.zoom(this.currentZoomLevel * zoomRatio, new Point2D(event.clientX, event.clientY));
        this.dispatchInteractionEvent('after', interaction, interactionDetail);
    ...
    dispatchInteractionEvent(timing, interaction, detail = {}) {
        ...
        switch (timing) {
            case 'before':
                type = 'before-interaction';
                break;
            case 'after':
                type = 'after-interaction';
                break;
    ...
    set currentZoomLevel(zoomLevel) {
        this.isInitialized.then(() => this.zoom(zoomLevel));
    }
    /**
     ...
     * @param maxZoomLevel
     */
    getZoomRangeSliderValue(zoomLevel = this.currentZoomLevel, minZoomLevel = this.minZoomLevel, maxZoomLevel = this.maxZoomLevel) {
        const numberOfSliderPoints = 100;
        const sliderValue = 1 + (numberOfSliderPoints - 1) * (Math.log(zoomLevel / minZoomLevel) / Math.log(maxZoomLevel / minZoomLevel));
     ...
     * @param maxZoomLevel
     */
    computeZoomValueFromRangeSlider(sliderValue, minZoomLevel = this.minZoomLevel, maxZoomLevel = this.maxZoomLevel) {
        const numberOfSliderPoints = 100;
        return minZoomLevel * Math.pow(maxZoomLevel / minZoomLevel, (sliderValue - 1) / (numberOfSliderPoints - 1));
    }
    /**
     ...
     */
    #computeZoom(zoomLevel, zoomPoint, sectionRect, oldZoomLevel, oldScrollPosition, oldScrollMargins, newScrollMargins) {
        zoomLevel = this.clampZoomLevel(zoomLevel);
        if (!newScrollMargins)
            newScrollMargins = this.#computeScrollMargins(zoomLevel);
     ...
     */
    zoom(zoomLevel, zoomPoint, sectionRect = this.getBoundingClientRect(), oldZoomLevel = this.currentZoomLevel, oldScrollPosition = this.scrollPosition, oldScrollMargins = this.#scrollMargins, dispatchEvents = true, forcedNewScrollMargins) {
        // If no zoom point is defined, use the center of the ScrollZoomBlock
        if (typeof zoomPoint === 'undefined') {
            zoomPoint = new Point2D(sectionRect.x + sectionRect.width / 2, sectionRect.y + sectionRect.height / 2);
        }
        const { zoomLevel: clampedZoomLevel, newScrollMargins, newScrollPosition } = this.#computeZoom(zoomLevel, zoomPoint, sectionRect, oldZoomLevel, oldScrollPosition, oldScrollMargins, forcedNewScrollMargins);
        if (dispatchEvents)
            this.dispatchZoomEvent('before', zoomPoint, oldZoomLevel, clampedZoomLevel);
        ...
        if (dispatchEvents)
            this.dispatchZoomEvent('after', zoomPoint, oldZoomLevel, clampedZoomLevel);
    }
    // MARK: smooth zoom
     ...
     */
    smoothZoom(zoomLevel, zoomPoint, zoomDuration = 200, // ms
    sectionRect = this.getBoundingClientRect(), oldZoomLevel = this.currentZoomLevel, oldScrollPosition = this.scrollPosition, oldScrollMargins = this.#scrollMargins) {
...
if (!customElements.get('scroll-zoom-block'))
    customElements.define('scroll-zoom-block', ScrollZoomBlock);
/**
