...
export class TranslationObserver {
     ...
     */
    serve(node, { init = true, method = 'attribute' } = {}) {
     ...
     */
    translate(container, strings, lang, defaultLang = this.defaultLang) {
        ...
        for (const e of [...container.querySelectorAll('[data-string]')]) {
            if (e instanceof HTMLImageElement)
                e.alt = getString(e.getAttribute('data-string') ?? '');
            else if (e instanceof Element)
                e.innerHTML = getString(e.getAttribute('data-string') ?? '');
        }
    ...
    notify(node, source, method) {
        ...
        if (!(node instanceof Element) || method === 'event' || method === 'both') {
            node.dispatchEvent(new CustomEvent('translate', { detail: { lang: language, language: language } }));
        }
    }
    /**
